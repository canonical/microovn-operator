...
def test_ovn_k8s_integration(juju: jubilant.Juju):
    ...
    juju.integrate("microovn","{}.ovn-central-k8s",format(juju_k8s.model))
    juju_k8s.wait(jubilant.all_active)
    juju.wait(jubilant.all_active)

    #ensure microovn central is down
    output = juju.exec("microovn status", unit="microovn/0")
    assert("central" not in output)
    #test ovn-nbctl still works which means its using ovn-k8s
    juju.exec("microovn.ovn-nbctl lr-add R1",unit="microovn/0")
    #check ovn-nbctl is using the same database across units
    output = juju.exec("microovn.ovn-nbctl show",unit="microovn/1")
    assert("R1" in output)
