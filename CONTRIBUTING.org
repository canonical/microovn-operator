...
The git revision the microovn charm was built from is recorded in a
~version~ file at the root of the charm, as produced by
~git describe --always --dirty~. Juju reports it as the charm version, so
the commit a deployed unit was built from can be found with:
#+begin_src shell
